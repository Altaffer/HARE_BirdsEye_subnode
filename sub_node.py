...
Postprocessing subscriber node to extract pre-recorded data from ROS2-based bags from field operations for BirdsEye implementation.

Listens to camera images, INS data, radar-altimeter readings, and IMU data, 
then synchonizes them into a unified data record to SQLite3 based database.
...
class subscriberNode(rclpy.node.Node):
    def __init__(self):
        ...
        self.declare_parameter("clicks_csv", "catch/data__2024_04_11.csv")
        # self.declare_parameter("clicks_csv", "catch/data__2024_04_12.csv")
        ...
        # added parameters for strobe pulse
        ...
        with open(self.sensors_yaml, 'r') as f:
            ...
            for device in devices:
                ...
                if device == self.sensor:
                    ...
                    self.K = data["intrinsics"]  # colmap estimates of Brown-Conrady model parameters
                    ...
                    extr = data["T_rad_imu"]  # extrinsic translation matrix between altimeter and imu
    ...
    def save_image_pose(self):
        # sanity check to validate RTK correction status corresponding to image and INS times 
    ...
    def cam_cb(self, msg: Image):
        ...
        # a strobe pulse is used to extract pose upon beginning of image exposure
        # ignore images and poses until the stobe is active and radalt is publishing depth
    ...
    def radalt_cb(self, msg: AltSNR):
        # snr must be greater than 13 for a valid message 
        if msg.snr > 13:
    ...
    def ins_cb(self, msg: DIDINS2):
        ...
        # validate that the strobe is active
